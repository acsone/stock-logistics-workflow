...
class StockReturnPicking(models.TransientModel):
    ...
    is_fee_customer_return = fields.Boolean(readonly=True)
    ...
    def default_get(self, fields):  # pylint: disable=redefined-outer-name
        ...
        if not (
            ...
            or "is_fee_customer_return" in fields
        ):
            return res
        picking = self.env["stock.picking"].browse(self.env.context["active_id"])
        charge_restocking_fee = picking.partner_id.charge_restocking_fee
        if "is_fee_customer_return" in fields:
            res["is_fee_customer_return"] = picking.is_fee_customer_out
    ...
    def _create_returns(self):
        ...
        self.ensure_one()
        if self.is_fee_customer_return:
