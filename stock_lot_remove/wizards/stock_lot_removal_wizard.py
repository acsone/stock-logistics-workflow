...
class StockLotRemovalWizard(models.TransientModel):
    ...
    def action_run(self):
        ...
        moves = self._create_moves_transfer_expired_lots(quants, reserved_moves)
        if moves:
    ...
    def _get_moves_vals(self, quant):
        """Get the values to create a transfer move for a quant."""
        name = _(
            "Moves expired lot %(lot_name)s (%(product_name)s)",
    ...
    def _create_moves_transfer_expired_lots(self, quants, moves_unreserved):
        """Create transfer of expired lots."""
        quants = quants.filtered(lambda q: q.quantity - q.reserved_quantity > 0)
        moves = self.env["stock.move"]
        if quants:
            moves_vals = [self._get_moves_vals(quant) for quant in quants]
            moves = moves.create(moves_vals)
        return moves
