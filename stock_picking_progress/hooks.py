...
def setup_move_line_progress(cr):
    ...
    cr.execute(
        """
        UPDATE stock_move_line SET progress =100
        WHERE state not in ('done', 'cancel')
        AND (reserved_qty IS NULL or reserved_qty = 0.0)
        """
    )
    cr.execute(
        """
...
def setup_move_progress(cr):
    ...
    cr.execute(
        """
        UPDATE stock_move SET progress =100
        WHERE state not in ('done', 'cancel')
        AND (product_uom_qty IS NULL or product_uom_qty = 0.0)
        """
    )
    cr.execute(
        """
