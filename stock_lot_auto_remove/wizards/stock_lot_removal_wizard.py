...
class StockLotRemovalWizard(models.TransientModel):
    ...
    warehouse_id = fields.Many2one(
        ...
        string="Warehouse",
        required=True,
        default=lambda self: self.env.context.get("active_model") == self._name
        and self.env.context.get("active_id", False),
    )

    description = fields.Html(
        compute="_compute_description",
        help="This wizard will automatically move quants of lot to remove "
        "from the selected warehouse to the designated destination location",
    )
    ...
    def _compute_description(self):
        ...
        for record in self:
            ...
            desc = _(
                "<p>This wizard will automatically move lots to remove <b>from</b "
                "location <b>%(orig_location)s</b> to <b>%(dest_location)s</b></p>"
                "<p>Removed lots are those with <b>%(field_descr)s before "
                "%(removal_date)s</b>.</p>",
                orig_location=record.warehouse_id.lar_orig_location_id.display_name,
                dest_location=record.warehouse_id.lar_dest_location_id.display_name,
    ...
    def _ensure_configuration(self):
        ...
        if (
            not self.warehouse_id.lar_orig_location_id
            or not self.warehouse_id.lar_dest_location_id
            or not self.warehouse_id.lar_picking_type_id
        ):
            raise ValidationError(
                _(
                    "Warehouse '%(wh_name)s' is not properly configured for expired "
                    "lot removal. Please set the origin and destination locations "
    ...
    def _get_move_to_unreserve_picking_domain(self, quants):
        ...
        return [
            ...
            ("picking_type_id", "!=", self.warehouse_id.lar_picking_type_id.id),
            ("printed", "=", False),
        ]
    ...
    def _get_transfer_move_vals(self, quant):
        ...
        return {
            ...
            "location_dest_id": self.warehouse_id.lar_dest_location_id.id,
    ...
    def get_transfer_picking_vals(self, quants, moves_unreserved):
        ...
        return {
            "picking_type_id": self.warehouse_id.lar_picking_type_id.id,
            "location_id": self.warehouse_id.lar_orig_location_id.id,
            "location_dest_id": self.warehouse_id.lar_dest_location_id.id,
