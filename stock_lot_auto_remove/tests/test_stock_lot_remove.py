...
class TestStockLotRemove(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.lot_2013 = cls.env["stock.lot"].create(
            {
                "name": "lot",
        ...
        cls.lot_2014 = cls.env["stock.lot"].create(
            {
                "name": "lot2",
                "product_id": cls.product.id,
        ...
        cls.env["stock.quant"]._update_available_quantity(
            cls.product, cls.loc_stock_child_1, 3, lot_id=cls.lot_2013
        )
        cls.env["stock.quant"]._update_available_quantity(
            cls.product, cls.loc_stock_child_2, 6, lot_id=cls.lot_2014
        )
        cls.customer_location = cls.env.ref("stock.stock_location_customers")
        cls.warehouse.lar_orig_location_id = cls.loc_stock
        cls.warehouse.lar_dest_location_id = cls.warehouse.wh_qc_stock_loc_id

        cls.quant_2013 = cls.env["stock.quant"].search(
            [
                ("product_id", "=", cls.product.id),
                ("lot_id", "=", cls.lot_2013.id),
    ...
    def _assert_initial_situation(self, picking):
        ...
        self.assertRecordValues(
            ...
            [
                {
                    "lot_id": self.lot_2013.id,
                    "reserved_uom_qty": 3,
                    "qty_done": 0,
                },
                {
                    "lot_id": self.lot_2014.id,
                    "reserved_uom_qty": 2,
                    "qty_done": 0,
                },
            ],
        ...
        self.assertEqual(lot_ids, self.lot_2013 | self.lot_2014)
    ...
    def test_remove_expired_lots_no_remove_wip(self):
        ...
        lot_2013_line = picking_out.move_line_ids.filtered(
            lambda ml: ml.lot_id == self.lot_2013
        )
    ...
    def test_remove_expired_lots_in_multiple_pickings(self):
        ...
        self.assertEqual(
            picking_remove.move_line_ids.lot_id, self.lot_2013 | self.lot_2014
        )
        self.assertEqual(
    ...
    def test_remove_expired_lots_in_multiple_pickings_no_remove_wip(self):
        ...
        lot_2014_line = picking_out.move_line_ids.filtered(
            lambda ml: ml.lot_id == self.lot_2014
        )
